...
function Document({ title, publishDate, headline }) {
  ...
  return (
    <div className="border border-gray-200 rounded-md p-4 mb-4 bg-white hover:shadow-md transition-shadow">
      ...
      <p className="text-base text-gray-700" dangerouslySetInnerHTML={{ __html: headline }} />
    </div>
  );
}
...
function Document2({ title, description, author, publishedDate, url, imageUrl }) {
	...
	return (
	  <a href={url} target="_blank" rel="noopener noreferrer" className="flex min-h-72 bg-gray-100 shadow-xl items-stretch mb-6 rounded-md overflow-hidden">
		<div className="border border-gray-200 w-40 h-40 flex-shrink-0">
		  {imageUrl ? (
		...
		<div className="flex-1 ml-4 p-4">
		  <h2 className="text-lg text-blue-600 font-medium mb-2">{title}</h2>
		  {author && <p className="text-sm text-gray-500 mb-1">Author: {author}</p>}
		  <p className="text-sm text-gray-500 mb-1">Published Date: {publishedDate}</p>
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gray-100">
      ...
      <main className="container mx-auto px-4 py-8 flex-grow">
        {showManual && (
          <div className="bg-white rounded-md shadow-md p-6 mb-8">
            ...
            <p className="text-gray-600 mb-2">1. Enter your search query in the input field.</p>
            ...
            <p className="text-gray-600 mb-2">4. Click on the title of a result to open a Google search for that article.</p>
            <p className="text-gray-600">5. Use the "Prev" and "Next" buttons to navigate through the pages of search results.</p>
          </div>
        )}
      ...
      <footer className="bg-gray-800 py-4">
        <div className="container mx-auto px-4 text-center text-gray-300">&copy; 2023 Codecatalysts</div>
      </footer>
    </div>
  );
}
